 ...
 * 1) Canvas lifecycle is managed manually to avoid React re-render overhead for particle updates.
 * 2) Particle system keeps mutable refs for animation state, minimizing per-frame allocations.
 * 3) Text rasterization to offscreen bitmap drives spawn targets for letter-shaped particle convergence.
 * 4) Phrase transitions blend hold time, dispersion, and regrouping to create readable motion cadence.
 * 5) Device-pixel-ratio scaling is applied to keep crisp rendering across Retina and standard displays.
 * 6) Resize handling recomputes layout and particles while preserving animation continuity.
 * 7) Frame loop uses requestAnimationFrame and explicit cancelation to prevent orphaned loops on unmount.
 * 8) Noise/jitter forces are tuned so particles feel organic without breaking legibility of text glyphs.
 * 9) Title-phase callback exposes deterministic checkpointing back to the landing orchestration.
 * 10) Visual timing is balanced for narrative impact while staying performant on lower-power devices.
 */
